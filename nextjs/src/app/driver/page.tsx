...
export function DriverPage() {
  ...
  const { data, error, isLoading} = useSwr<Route[]>('http://localhost:3000/routes', fetcher, { fallbackData: [] });
  ...
  async function startRoute() {
    const routeId = (document.getElementById('route') as HTMLInputElement).value;
  ...
  return (
    <Grid2 container sx={{ display: 'flex', flex: 1 }}>
      <Grid2 xs={4} px={2}>
        <Typography variant='h6'>Minha viagem</Typography>
